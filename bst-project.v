(** * Projeto: Binary Search Trees (BST) *)

(** Neste projeto estudaremos as árvores de busca binária em um contexto mais formal. Com o intuito de aproveitar o esforço feito para realização do primeiro projeto, aqui o trabalho também poderá ser feito no assistente de provas Coq, mas se você preferir pode implementar a estrutura de árvores de busca binária em sua linguagem de programação preferida e provar as propriedades em papel e lápis.

Este projeto foi adaptado de https://softwarefoundations.cis.upenn.edu/

Leitura recomendada:

    - Capítulo 12 de _Introduction to Algorithms, 3rd Edition_, Cormen, Leiserson, and Rivest, MIT Press 2009. *)
...
(** Utilizaremos números naturais para representar as chaves em cada nó de nossas BSTs porque os naturais possuem uma ordem total [<=?] com diversos teoremas já provados. *)

Definition key := nat.

(** Definiremos indutivamente árvores cujos elementos têm tipo [V] qualquer. Nesta construção, cada nó da árvore possui uma chave [k] que está relacionada com um valor [v] do tipo [V]. Árvores possuem dois construtores, [E] que representa a árvore vazia, e [T] que recebe os seguintes argumentos: 
...
(** Uma BST é caracterizada pela seguinte invariante: dado qualquer nó não-vazio com chave [k], todas as chaves da subárvore à esquerda são menores do que [k], e todas as chaves da subárvores à direita são maiores do que [k]. Baseado nesta invariante, construímos a função [bound k t] que retorna [true] quando [k] é uma chave de [t], e [false] caso contrário. *)
...
(** A função [lookup d k t] retorna o valor ligado à chave [k] em [t], ou retorna
    o valor _default_ [d] quando [k] não é uma chave de [t]. Esta função também é baseada na invariante de BST. *)
...
(** Assim, se [t] não é uma BST então as funções [bound] e [lookup] podem retornar respostas erradas: *)

Module NotBst.
  Open Scope string_scope.

  (** De fato, considere a seguinte árvore que não é uma BST: *)
...
(** A função [insert k v t] insere a chave [k] e o valor [v] na árvore [t], de forma a preservar a invariante de uma BST. *)
...
(** Nossa primeira tarefa será mostrar que a função [insert] de fato preserva esta invariante. Vamos então formalizar a invariante de uma BST. Faremos isto com a ajuda da função [ForallT]: *)
...
(** Esta função expressa as condições para que uma árvore satisfaça uma dada propriedade [P]: Se a árvore for vazia então a propriedade é automaticamente satisfeita por vacuidade, e portanto retorna [True]. Quando a árvore não é vazia, e portanto tem a forma [T l k v r] então precisamos que o nó que tem chave [k] e valor [v] satisfaça a propriedade [P], assim como as subárvores à esquerda e à direita.

Agora vamos definir a invariante BST, que é composta de dois construtores:

    - O primeiro, denotado por [BST_E], consiste em um axioma que estabelece que uma árvore vazia é uma BST.

    - O segundo, denotado por [BST_T], consiste na regra que diz que para que uma árvore não vazia [T l x v r] seja uma BST é necessário que todas as chaves da subárvore à esquerda sejam menores do que [x], todas a chaves da subárvore à direita sejam maiores do que [x], e que as subárvores à esquerda e à direita sejam também BSTs: *)
...
(** ** Questão 1: *)
(** A primeira questão consiste em provar que uma árvore vazia (de qualquer tipo) é uma BST. *)
...
(** ** Questão 2:  *)
(** A seguir, provaremos que a função [insert] produz uma BST, mas antes prove que a função [insert] preserva qualquer propriedade dos nós: *)
...
(** ** Questão 3:  *)
(** Prove que ao receber uma BST como argumento, a função [insert] gera uma BST. *)
...
(** * Correção das funções [lookup] e [bound] *)

(** Qual o comportamento esperado para as funções [lookup] e [bound]? *)

(** Ao procurarmos uma chave na BST vazia com a função [lookup], devemos obter o valor default [d]: *)
...
(** Se inserirmos a chave [k] e valor [v] na BST [t], então a busca em [insert k v t] com a função [lookup] retorna [v]: *)
...
(** ** Questão 4: *)
(** Prove que uma busca de uma chave [k'], via a função [lookup], na BST [insert k v t] com [k] e [k'] distintos retorna o resultado de buscar [k'] em [t]:  *)
...
(** Podemos converter uma BST em uma lista cujos elementos são pares [(k,v)] contendo a chave [k] e o valor [v]. Se a lista estiver ordenada pelas chaves, então duas árvores representando a mesma BST são convertidas na mesma lista pela função [elements]: *)
...
(** Agora vamos provar que esta transformação possui algumas propriedades interessantes. Por exemplo, dada uma BST [t], uma chave [k] associada a um valor [v] ocorre em [t] se, e somente se o par [(k,v)] é um elemento de [elements t]. Dividiremos esta prova em duas partes: *)
...
A prova da correção exige um pouco mais de trabalho porque enquanto [BST] utiliza o predicado [ForallT] para garantir que todos os nós da árvore satisfazem um determinada propriedade, listas utilizam o predicado [Forall] para isto. Assim, precisamos estabelecer uma relação entre estes predicados. *)
